...
Notes under Discover are presented based on their scores in a descending rank list. Navigate to next ore previous note either using the toolbar buttons or keyboard shortcuts (`a`/`d`, or left/right keys):

![](/images/discoverLight.png)

You can filter notes to discover only certain notebooks or tags:

![](/images/discoverFilterLight.png)

Scores are calculated based on several factors, and their weight can be adjusted under settings:
...
Notes can be rated from 0 to 5 (by click or keyboard shortcuts `jkl;'`). Higher rated notes are scored higher. Think of rating as more of a longterm score modifier (e.g. I like this note and will continue to like this note).
...
Under Discover, notes can also be given upvote or downvote (`w`/`s`, or up/down keys). By default, every note starts with vote of 5. Use weight as more of a short term score modifier (e.g., I like this note but just want to see it less).

Adjust its weight (between 0 and 10) to affect how heavily a higher voted note will be scored.

### Penalty Window and Penalty Recovery Window
...
Curator will recalculate scores for notes at startup and every few hours to mitigate this problem. By default (0 day), all notes have their scores recalculated on startup. When the collection size is large enough, this may cause performance problems. For example, I currently have about 8,000 notes, and it takes about 2.5 seconds to calculate all of their scores.
...
### Score Refresh Frequncy

This will determine how often note scores are recalculated.
